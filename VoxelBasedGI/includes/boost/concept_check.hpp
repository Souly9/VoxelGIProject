...
namespace boost
{
	...
	void function_requires(Model* = nullptr)
	{
...
# if defined(BOOST_HAS_LONG_LONG)
	template <>
	struct Integer<long_long_type>
	{
	};

	template <>
	struct Integer<ulong_long_type>
	{
	};
...
# if defined(BOOST_HAS_LONG_LONG)
	template <>
	struct SignedInteger<long_long_type>
	{
	};
# elif defined(BOOST_HAS_MS_INT64)
...
# if defined(BOOST_HAS_LONG_LONG)
	template <>
	struct UnsignedInteger<ulong_long_type>
	{
	};
# elif defined(BOOST_HAS_MS_INT64)
...
#define BOOST_DEFINE_BINARY_OPERATOR_CONSTRAINT(OP,NAME)    \
  ...
  private:                                                  \
	...
	{
		using argument_type = typename Func::argument_type;
		using result_type = typename Func::result_type;
    ...
    : BinaryFunction<
		...
		>
	{
		using first_argument_type = typename Func::first_argument_type;
		using second_argument_type = typename Func::second_argument_type;
		using result_type = typename Func::result_type;
	...
	{
		using value_type = typename std::iterator_traits<TT>::value_type;
		using difference_type = typename std::iterator_traits<TT>::difference_type;
		using reference = typename std::iterator_traits<TT>::reference;
		using pointer = typename std::iterator_traits<TT>::pointer;
		using iterator_category = typename std::iterator_traits<TT>::iterator_category;
      ...
      , ReversibleContainer<C>
	{
		...
		{
			using iterator = typename Mutable_ForwardContainer<C>::iterator;
	...
	{
	private:
		using self = Mutable_RandomAccessContainer;
	public:
      ...
      , DefaultConstructible<C>
	{
		...
		using value_compare = typename C::value_compare;
		using iterator = typename C::iterator;
		...
		BOOST_CONCEPT_USAGE(AssociativeContainer)
		{
			...
			using value_type_ = typename AssociativeContainer::value_type;
    ...
    : AssociativeContainer<C>
	{
		BOOST_CONCEPT_USAGE(PairAssociativeContainer)
		{
			...
			using required_value_type = std::pair<const key_type, mapped_type>;
	...
	private:
		...
		using self = SortedAssociativeContainer;
		iterator p;
		const_iterator cp;
		std::pair<iterator, iterator> r;
		std::pair<const_iterator, const_iterator> cr;
	...
	BOOST_concept(Collection, (C))
	{
		...
		{
			boost::function_requires<InputIteratorConcept<iterator>>();
			boost::function_requires<InputIteratorConcept<const_iterator>>();
			boost::function_requires<CopyConstructibleConcept<value_type>>();
			const_constraints(c);
			i = c.begin();
			i = c.end();
			c.swap(c);
