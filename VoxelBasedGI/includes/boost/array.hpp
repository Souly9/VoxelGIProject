...
namespace boost
{
	...
	public:
		...
		{
			return i >= size() ? throw_exception(std::out_of_range("array<>: index out of range")), true : true;
		}
	};
...
//      borland compilers can't handle this form. 
   ...
   typename detail::c_array<T,N>::type const& get_c_array(const boost::array<T,N>& arg)
   {
	...
	T& get(array<T, N>& arr) BOOST_NOEXCEPT {
		BOOST_STATIC_ASSERT_MSG(Idx < N, "boost::get<>(boost::array &) index out of range");
		return arr[Idx];
	}

	template <size_t Idx, typename T, size_t N>
	const T& get(const array<T, N>& arr) BOOST_NOEXCEPT {
